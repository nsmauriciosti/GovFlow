...
const InvoiceRow = React.memo(({ 
  ...
  return (
    ...
    >
      <td className={`p-3 lg:p-4 text-xs font-bold border-b border-slate-100 dark:border-slate-800 whitespace-nowrap ${isCancelado ? 'text-slate-400' : 'text-slate-700 dark:text-slate-200'}`}>{invoice.secretaria}</td>
      <td className={`p-3 lg:p-4 text-xs border-b border-slate-100 dark:border-slate-800 min-w-[150px] ${isCancelado ? 'text-slate-400' : 'text-slate-600 dark:text-slate-400'}`}>{invoice.fornecedor}</td>
      <td className="p-3 lg:p-4 text-xs border-b border-slate-100 dark:border-slate-800">
        <div className={`font-mono text-[9px] lg:text-[10px] ${isCancelado ? 'text-slate-300' : 'text-slate-500 dark:text-slate-500'}`}>NE: {invoice.ne}</div>
        <div className={`font-mono text-[9px] lg:text-[10px] ${isCancelado ? 'text-slate-300' : 'text-slate-500 dark:text-slate-500'}`}>NF: {invoice.nf}</div>
      </td>
      ...
      <td className="p-3 lg:p-4 border-b border-slate-100 dark:border-slate-800">
        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black border uppercase tracking-tight transition-colors ${getStatusBadgeClass(invoice.situacao)}`}>
          {invoice.situacao}
        </span>
      </td>
      <td className="p-3 lg:p-4 border-b border-slate-100 dark:border-slate-800" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-center gap-1.5 lg:gap-2">
          {canWrite ? (
            <>
              <button onClick={() => onToggleStatus(invoice.id)} disabled={isCancelado} title="Liquidar" className={`p-1.5 rounded-lg transition-all ${isCancelado ? 'text-slate-200 dark:text-slate-700 cursor-not-allowed' : 'text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-950/40'}`}>
              ...
              <button onClick={() => onDelete(invoice.id)} title="Excluir" className="p-1.5 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition-all">
            ...
            <div className="text-[8px] text-slate-300 uppercase font-black">Consulta</div>
          )}
        </div>
      </td>
    </tr>
...
const InvoiceTable: React.FC<InvoiceTableProps> = ({ invoices, onDelete, onToggleStatus, onSelectInvoice, onEditInvoice, currentUser, theme }) => {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col relative transition-colors">
      {isPending && (
        ...
        <table className="w-full text-left border-collapse min-w-[900px]" role="table">
          ...
          <tbody className="divide-y divide-slate-100 dark:divide-slate-800 transition-colors">
            {paginatedInvoices.length === 0 ? (
              <tr>
                <td colSpan={8} className="p-16 text-center text-slate-400 font-medium italic text-sm">
      ...
      <div className="px-4 py-4 bg-slate-50 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row justify-between items-center gap-4 no-print transition-colors">
        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4">
          <p className="text-[10px] sm:text-xs text-slate-500 font-bold uppercase tracking-tight">
            <span className="text-indigo-600 dark:text-indigo-400 font-black">{startRange}-{endRange}</span> de {invoices.length}
          </p>
          <div className="flex items-center gap-2">
            <label className="text-[9px] font-black text-slate-400 uppercase">Ver:</label>
            <select 
              className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-[10px] font-black p-1 outline-none text-slate-900 dark:text-slate-100"
              value={itemsPerPage}
        ...
        <div className="flex items-center gap-1.5">
          <button onClick={() => handlePageChange(1)} disabled={currentPage === 1} className="p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-white dark:hover:bg-slate-800 disabled:opacity-30 transition-all">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M11 19l-7-7 7-7m8 14l-7-7 7-7" /></svg>
          </button>
          
          <div className="flex items-center px-3 gap-2">
            <span className="text-xs font-black text-indigo-600 bg-indigo-50 dark:bg-indigo-950/40 px-3 py-1 rounded-lg border border-indigo-100 dark:border-indigo-900/50">{currentPage}</span>
            <span className="text-[9px] font-black text-slate-400 uppercase">/ {totalPages || 1}</span>
          </div>

          <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages || totalPages === 0} className="p-1.5 rounded-lg text-slate-400 hover:text-indigo-600 hover:bg-white dark:hover:bg-slate-800 disabled:opacity-30 transition-all">
      ...
      <style>{`
        ...
        .scrollbar-thin::-webkit-scrollbar-thumb { background: ${theme === 'dark' ? '#334155' : '#e2e8f0'}; border-radius: 10px; }
      `}</style>
    </div>
  );
};
